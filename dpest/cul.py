...
def cul(
    ...
    **Examples:**
    ...
    try:
        ...
        # Construct the path to package-level defaults from dpest/arguments.yml
        arguments_file = os.path.join(current_dir, 'arguments.yml')

        # Ensure the package-level defaults from dpest/arguments.yml file exists
        if not os.path.isfile(arguments_file):
            raise FileNotFoundError(f"YAML file not found: {arguments_file}")

        # Load package-level defaults from dpest/arguments.yml configuration
        ...
        if parameters_grouped == {}:
            ...
            # 1) Get the 5-char genotype key from the CUL filename (e.g. WHCER)
            cul_basename = os.path.basename(validated_cul_file_path)  # WHCER048.CUL
            genotype_key = cul_basename[:5]  # WHCER
            ...
            try:
                ...
                raise ValueError(
                    "Automatic detection of cultivar parameter groups is not available "
                    f"for the CUL file '{cul_basename}' (genotype key '{genotype_key}').\n\n"
                    "Please provide the 'parameters_grouped' argument explicitly when "
                    "calling cul(), for example:\n"
                    "    cul(..., P='P1V, P1D', G='G1', PHINT='PHINT')"
                ) from exc
            ...
            # 4) Load CULTIVAR_TPL_FILE.PARAMETERS from that crop/model file
            with open(crop_model_arguments_file, "r") as cm_yml:
                crop_model_yaml_data = yaml.safe_load(cm_yml)

            try:
                raw_parameters_grouped = crop_model_yaml_data[yml_cultivar_block][yaml_parameters]
            except KeyError as exc:
                raise ValueError(
                    "No default cultivar parameter groups are defined for this crop/model.\n\n"
                    "To use cul() with this CUL file, you must provide the "
            ...
            parameters_grouped = {key: ", ".join(value) for key, value in raw_parameters_grouped.items() }
        ...
        def extract_parameter_values(line_number):
            ...
            for parameter in parameters:
                try:
                    ...
                    raise ValueError(
                        f"Parameter '{parameter}' does not exist in the header line of {validated_cul_file_path}.")
        ...
        output_new_file_path = os.path.join(output_path, os.path.splitext(os.path.basename(validated_cul_file_path))[
            0] + '_CUL' + '.' + new_template_file_extension)
