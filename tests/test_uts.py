...
def test_invalid_treatment_type(capsys):
    plantgro_path = Path('tests/DSSAT48/Wheat/PlantGro.OUT').resolve()
    result = dpest.uts(str(plantgro_path), 123, ['LAID'])
    captured = capsys.readouterr()
...
def test_invalid_variables_type(capsys):
    plantgro_path = Path('tests/DSSAT48/Wheat/PlantGro.OUT').resolve()
    result = dpest.uts(str(plantgro_path), 'Valid Treatment', None)
    captured = capsys.readouterr()
...
def test_file_not_found(capsys):
    ...
    result = dpest.uts(str(non_existent_path), 'Valid Treatment', ['LAID'])
    captured = capsys.readouterr()
