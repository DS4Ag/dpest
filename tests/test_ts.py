...
####################
def test_ts_with_optional_parameters(tmp_path):
...
def test_ts_variable_filtering(tmp_path):
...
def test_ts_missing_treatment_argument(tmp_path, capsys):
...
def test_ts_special_characters_in_treatment(tmp_path, capsys):
...
def test_ts_variables_accepts_string(tmp_path):
...
def test_ts_invalid_suffix(tmp_path, suffix_value, error_msg, capsys):
    repo_root = Path(__file__).parent.parent
    plantgro_file = repo_root / "tests/DSSAT48/Wheat/PlantGro.OUT"
...
def test_ts_file_not_found(tmp_path, capsys):
...
def test_ts_missing_yaml_file(tmp_path, capsys):
...
def test_ts_invalid_markers(tmp_path, mrk, smk, expected_error, capsys):
    repo_root = Path(__file__).parent.parent
    plantgro_file = repo_root / "tests/DSSAT48/Wheat/PlantGro.OUT"
...
def test_ts_duplicate_markers(tmp_path, capsys):
...
def test_ts_mrk_smk_same_character(tmp_path, capsys):
...
def test_ts_different_output_formats(tmp_path):
    repo_root = Path(__file__).parent.parent
    plantgro_file = repo_root / "tests/DSSAT48/Wheat/PlantGro.OUT"
